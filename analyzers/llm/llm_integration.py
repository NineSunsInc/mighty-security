...
from .base_analyzer import BaseLLMAnalyzer, LLMRequest, AnalysisType
...
class LLMAnalysisCoordinator:
    ...
    def __init__(self, llm_provider: str = "cerebras", api_key: Optional[str] = None):
    ...
    async def _analyze_file_with_ml(self, file_path: str, content: str) -> Optional[Any]:
    ...
    def _load_file_contents(self, repo_path: Path, static_results: Dict) -> Dict[str, str]:
        ...
        if len(files_analyzed) < 5:
            try:
                from analyzers.shared_constants import get_scannable_files
            except ImportError:
                # Try relative import
                import sys
                from pathlib import Path
